...
class TodoList extends Component {
  ...
  addBtnPressed(pr) {
    let array = this.state.list.filter((data) => data.todo === this.state.value);
    if (this.state.value.length > 0 && array.length === 0) {
  ...
  render() {
    ...
    return (
      <div align="center">
        ...
        <FormControl className="inputTodo" placeholder="Search todo.." onChange={this.handleSearchChange} />
        <AddTodo handleChange={this.handleChange} addBtnPressed={this.addBtnPressed} value1={this.state.value}/>
